...
import com.wenge.data.entry.common.CommonConstants;
...
public class DateTimeUtils {
     ...
     * @return date
     */
    public static Date getDateByDate (Date date, int count) {
     ...
     *            时间字符串
     * @param format
     *            时间字符串格式（如str为：2018-05-29 12:12:00; 则format即为：yyyy-MM-dd HH:mm:ss）
     * @return date
     */
     ...
     * @return date
     */
    public static Date getDayOfWeek (Date date, int count) {
        if (date == null) {
            date = dateLocal.get();
        }
        Calendar ca = calendarLocal.get();
     ...
     * @return
     */
    public static long getEndOfDay (String str, String format) {
        Date date = null;
        if (StringUtils.isEmpty(str)) {
            date = dateLocal.get();
        } else {
        ...
        String newTime = getStringByDate(date, CommonConstants.SIMPLE_DATE_FORMAT);
        date = getDateByString(newTime, CommonConstants.SIMPLE_DATE_FORMAT);
        return date.getTime() / 1000L + 86399L;
    }
     ...
     * @return
     */
    public static long getLongByString (String str, String format) {
        long longdate = 0L;
        if (StringUtils.isEmpty(str)) {
     ...
     * @return
     */
    public static long getStartOfDay (String str, String format) {
        Date date = null;
        if (StringUtils.isEmpty(str)) {
            date = dateLocal.get();
        } else {
     ...
     * @return
     */
    public static String getStringBylong (long longTime, String format) {
        Date date = dateLocal.get();
        if (longTime != 0) {
            date.setTime(longTime * 1000L);
        }
     ...
     * @return
     */
    public static List<String> getSteOfDateTimeInterval(Date startDate, Date endDate, String format) {
        List<String> list = new ArrayList<>();
        if (startDate != null && endDate != null) {
            while (startDate.before(endDate)) {
                String dateStr = DateTimeUtils.getStringByDate(startDate, format);
                list.add(dateStr);
                startDate = DateTimeUtils.getDateByDate(startDate, 1);
            }
        }
        return list;
    }
